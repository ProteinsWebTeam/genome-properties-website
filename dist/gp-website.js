var GenomePropertiesWebsite = (function () {
  ...
  var parseGenProp = function parseGenProp(txt) {
    ...
    txt.split('\n').forEach(function (line) {
      ...
      if (key in mainKeys) property[mainKeys[key]] = value; // References
      ...
      if (key === 'CC') property.description += value + ' '; // Notes

      if (key === '**') property.notes += value + ' '; // Steps
      ...
      if (key in stepKeys && currentStep) currentStep[stepKeys[key]] = value; // Evidence
    ...
    return property; // console.log(property);
  ...
  function () {
    function RstRenderer(server, manager) {
      _classCallCheck(this, RstRenderer);
    ...
    _createClass(RstRenderer, [{
            ...
            if (line.indexOf('  ') === 0 && line.trim() !== '') {
              ...
              switch (objBlock.type) {
                ...
                case blockType.INCLUDE:
                  return _this.manager.getResource("INCLUDE_".concat(objBlock.value), "".concat(_this.server, "/docs/").concat(objBlock.value), _this.markup2html.bind(_this), "INCLUDE_".concat(objBlock.value), false);
      ...
      value: function applySubstitutions(txt) {
        ...
        for (var key in this.substitutions) {
          ...
          if (value.type == "image") {
            replacement = "<img src=\"".concat(this.server, "/docs/").concat(value.value, "\" ").concat(this.renderHTMLAtrributes(value.attributes, ['alt', 'target']), "/>");
            if ("target" in value.attributes) replacement = "<a href=\"".concat(value.attributes.target, "\" ").concat(this.renderHTMLAtrributes(value.attributes, ['value', 'target']), ">").concat(replacement, "</a>");
          } else {
            replacement = value.value;
          } // console.log(`|${key}|`,replacement);


          newText = newText.replace("|".concat(key, "|"), replacement);
        }
      ...
      value: function addHyperLinks(txt) {
        var re = /`([a-zA-Z_\-:@\/\.\d\s]+)<([a-zA-Z_\-:@#\/\.\d?=&]+)>`_/g;
  ...
  var searchHierarchy = function searchHierarchy(term) {
    ...
    if (term.trim() !== '') document.querySelectorAll("span.genprop-label[text*=\"".concat(term, "\"]")).forEach(function (e) {
  ...
  var renderGenPropHierarchy = function renderGenPropHierarchy(hierarchy) {
    ...
    return "\n  <div class=\"genome-property ".concat(expanded ? 'expanded' : '', "\">\n    <header>\n      ").concat(!hierarchy.children.length ? '・' : "\n      <a style=\"border: 0;color: darkred;\" class=\"expander\">".concat(expanded ? '▾' : '▸', "</a>\n      "), "\n      <span class=\"genprop-label\" text=\"").concat(hierarchy.id, " ").concat(hierarchy.name, "\">\n        <a href=\"#").concat(hierarchy.id, "\">").concat(hierarchy.id, "</a>:\n        <a href=\"#").concat(hierarchy.id, "\">").concat(hierarchy.name, "</a>\n      </span>\n    </header>\n    ").concat(!hierarchy.children.length ? '' : "\n      <div class=\"children\" style=\"\n        margin-left: ".concat(level * 10, "px;\n      \">\n        ").concat(hierarchy.children.map(function (child) {
  ...
  function () {
    function GenPropRenderer() {
      _classCallCheck(this, GenPropRenderer);
    }

    _createClass(GenPropRenderer, [{
      key: "renderGenProp",
      value: function renderGenProp(property) {
        var _this = this;

        var isCategory = property.type === "CATEGORY";
        return "\n      <h2>".concat(property.accession, " - ").concat(property.name, "</h2>\n      <span class=\"tag\">Type: ").concat(property.type, "</span>\n      <br/><br/>\n      <div>Author: ").concat(property.author, "</div>\n      <br/>\n      <div>\n        <h4>Description</h4>\n        <p>").concat(this.renderDescription(property.description, property.accession), "</p>\n      </div>\n      <div>\n        <h4>References</h4>\n        ").concat(property.references && property.references.length ? this.renderReferences(property.references, property.accession) : "<cite>None</cite>", "\n      </div>\n      <div>\n        <h4>Database Links</h4>\n        ").concat(property.databases && property.databases.length ? "\n          <ul>\n            ".concat(property.databases.map(function (db) {
          return "\n              <li>".concat(_this.renderDatabaseLink(db.title, db.link), "</li>\n            ");
        }).join(""), "\n          </ul>\n          ") : "<cite>None</cite>", "\n      </div>\n      <div>\n        ").concat(isCategory ? this.renderChildren(property) : this.renderSteps(property), "\n      </div>\n      <br/>\n    ");
      }
    }, {
      key: "renderDescription",
      value: function renderDescription(txt, acc) {
        return txt.replace(/\[(\d+)\]/g, "<a href=\"#".concat(acc, "-$1\">[$1]</a>"));
      }
    }, {
      key: "renderReferences",
      value: function renderReferences(references, accession) {
        return "\n      <ul class=\"references\">\n        ".concat(references.map(function (ref) {
          return "\n            <li class=\"reference\" id=\"".concat(accession, "-").concat(ref.number, "\">\n              <span class=\"index\">[").concat(ref.number, "]</span>\n              <span class=\"authors\">").concat(ref.author, "</span>\n              <span class=\"title\">").concat(ref.title, "</span>\n              <span class=\"citation\">").concat(ref.citation, "</span>\n              <span class=\"reference_id\">").concat(ref.PMID, "</span>\n              <a target=\"_blank\" rel=\"noopener\" href=\"https://europepmc.org/abstract/MED/").concat(ref.PMID, "\">EuropePMC</a>\n            </li>\n        ");
        }).join(""), "\n      </ul>\n    ");
      }
    }, {
      key: "renderChildren",
      value: function renderChildren(property) {
        ...
        return "\n    <div>\n      <h4>Genome properties</h4>\n      <table class=\"no-stripe\" style=\" background-color:#86a5bb;\">\n        <tr style=\"background-color: #ddd\">\n          <th width=\"30%\">Property</td>\n          <th style=\"text-align: left;\">Accession</th>\n        </tr>\n\n        ".concat(property.steps.map(function (step, j) {
          return "\n          <tr style=\"background-color: ".concat(j % 2 == 0 ? "white" : "#eee", "\">\n            <td rowspan=\"").concat(step.evidence_list.length, "\">\n              ").concat(step.number, ". ").concat(_this2.getFirstEvidenceLink(step.evidence_list, step.id), "\n            </td>\n              ").concat(step.evidence_list.map(function (e, i) {
            return "\n                ".concat(i > 0 ? "<tr style=\"background-color: ".concat(j % 2 == 0 ? "white" : "#eee", "\">") : "", "\n                  <td>").concat(_this2.renderEvidence(e.evidence), "</td>\n                ").concat(i > 0 ? "</tr>" : "", "\n              ");
          }).join(""), "\n          </tr>\n        ");
        }).join(""), "\n      </table>\n    </div>\n    ");
      }
    }, {
      ...
      value: function renderSteps(property) {
        ...
        return "\n    <div>\n      <h4>Steps</h4>\n      <table class=\"no-stripe\" style=\" background-color:#86a5bb;\">\n        <tr style=\"background-color: #ddd\">\n          <th width=\"30%\">Step</td>\n          <th style=\"text-align: left;\">Evidence</th>\n          <th style=\"text-align: left;\">Go Terms</th>\n        </tr>\n\n        ".concat(property.steps.map(function (step, j) {
          return "\n          <tr style=\"background-color: ".concat(j % 2 == 0 ? "white" : "#eee", "\">\n            <td rowspan=\"").concat(step.evidence_list.length || 1, "\">").concat(step.number, ". ").concat(step.id, "\n              ").concat(step.requires !== "1" ? '<br/><span class="tag secondary">Optional</span>' : "", "\n            </td>\n              ").concat(step.evidence_list.map(function (e, i) {
            return "\n                ".concat(i > 0 ? "<tr style=\"background-color: ".concat(j % 2 == 0 ? "white" : "#eee", "\">") : "", "\n                  <td>").concat(_this3.renderEvidence(e.evidence), "</td>\n                  <td>").concat(_this3.renderEvidence(e.go), "</td>\n                ").concat(i > 0 ? "</tr>" : "", "\n              ");
          }).join(""), "\n              ").concat(step.evidence_list.length ? "" : "<td><cite>None</cite></td><td><cite>None</cite></td>", "\n          </tr>\n        ");
        }).join(""), "\n      </table>\n      <span\n        data-tooltip\n        aria-haspopup=\"true\"\n        data-disable-hover=\"false\"\n        class=\"has-tip tag secondary\"\n        title=\"The threshold value is the number of required (non-optional) steps found, above which a value of PARTIAL is set for the property. Partial can be thought of as some evidence of the property. A full YES state is only defined where ALL required steps are found.\"\n      >\n          Threshold: ").concat(property.threshold, "\n      </span>\n    </div>\n    ");
      }
    }, {
      ...
      value: function renderDatabaseLink(title, link) {
        ...
        if (parts[0] === "KEGG") a = "<a\n        href=\"http://www.genome.jp/dbget-bin/www_bget?pathway:".concat(parts[1], "\"\n      >KEGG</a>");else if (parts[0] === "IUBMB") a = "<a\n        href=\"http://www.chem.qmul.ac.uk/iubmb/enzyme/reaction/".concat(parts[1], "/").concat(parts[2], ".html\"\n      >IUBMB</a>");else if (parts[0] === "MetaCyc") a = "<a\n        href=\"https://metacyc.org/META/NEW-IMAGE?type=NIL&object=".concat(parts[1], "\"\n      >MetaCyc</a>");else if (parts[0] === "Complex Portal") a = "<a\n        href=\"https://www.ebi.ac.uk/complexportal/complex/".concat(parts[1], "\"\n      >Complex Portal</a>");else if (parts[0] === "PDBe") a = "<a\n        href=\"https://www.ebi.ac.uk/pdbe/entry/pdb/".concat(parts[1], "\"\n      >PDBe</a>");
        return "<b>".concat(title, "</b>: ").concat(a);
      }
    }, {
      ...
      value: function getFirstEvidenceLink(evidence_list, text) {
        if (evidence_list && evidence_list.length) {
          ...
          return "<a href=\"#".concat(gp, "\">").concat(text, "</a>");
        }
      ...
      value: function renderEvidence(txt) {
        ...
        return parts.filter(function (p) {
          ...
          var term = t.trim();
          if (term.startsWith("GO:")) return "<a href=\"http://www.ebi.ac.uk/QuickGO/GTerm?id=".concat(term, "\">").concat(term, "</a>");
          if (term.startsWith("GenProp")) return "<a href=\"#".concat(term, "\">").concat(term, "</a>");
          if (term.startsWith("IPR")) return "<a href=\"https://www.ebi.ac.uk/interpro/entry/".concat(term, "\">").concat(term, "</a>");
          if (term.startsWith("TIGR")) return "<a href=\"http://www.jcvi.org/cgi-bin/tigrfams/HmmReportPage.cgi?acc=".concat(term, "\">").concat(term, "</a>");else {
            return term;
          }
        }).join(" - ");
      }
  ...
  function () {
    function ViewerRenderer(github) {
      ...
      _classCallCheck(this, ViewerRenderer);

      this.github = github;
      this.options = options;
    }

    _createClass(ViewerRenderer, [{
      key: "getViewerHTML",
      ...
      value: function loadViewer() {
        var d3 = gpv.d3,
          ...
          server: "".concat(this.github, "/flatfiles/gp_assignments/SUMMARY_FILE_{}.gp"),
          hierarchy_path: "".concat(this.github, "/flatfiles/hierarchy.json"),
          server_tax: "".concat(this.github, "/flatfiles/taxonomy.json"),
          model_species_path: "".concat(this.github, "/flatfiles/gp_assignments/json/JSON_MERGED"),
        ...
        d3.select("#toggle-tax").on("click", function (d, i, c) {
  ...
  function () {
    ...
    _createClass(GenomePropertiesWebsite, [{
      ...
      value: function getGenProp(acc) {
        ...
        var base = "".concat(this.github, "/data/").concat(acc, "/");
      ...
      value: function getHome() {
        ...
        return this.getResource("#home", "".concat(this.github, "/docs/landing.rst?"), function (text) {
      ...
      value: function getContentTabs(resource, tabs, renderer) {
        ...
        if (location.hash !== "#viewer") {
          body = location.hash === "#hierarchy" || location.hash === "#browse" ? this.getProps() : this.getResource(location.hash, "".concat(this.github).concat(resource[location.hash]), renderer);
        }
        ...
        return "\n      <ul class=\"tabs\" data-tabs id=\"browse-tabs\">\n      ".concat(tabs.map(function (tab) {
          ...
          return "\n          <li class=\"tabs-title ".concat(isActive ? "is-active" : "", "\" >\n            <a ").concat(isActive ? 'aria-selected="true"' : "", " href=\"").concat(hash, "\">").concat(tab, "</a>\n          </li>");
        }).join(""), "\n      </ul>\n      <br/>\n      ").concat(body, "\n    ");
      }
    }, {
      key: "renderStatsFile",
      value: function renderStatsFile(txt) {
        return "\n    <div id=\"content-browse-tab\">\n      <ul>\n        ".concat(txt.split("\n").map(function (line) {
          if (line.trim() === "") return "";
          var gp = line.split("\t");
          return "\n            <li>\n              <a href=\"#".concat(gp[0], "\">").concat(gp[0], "</a>:\n              <a href=\"#").concat(gp[0], "\">").concat(gp[1], "</a>\n            </li>");
        }).join(""), "\n      </ul>\n    </div>");
      }
    }, {
      key: "getProps",
      value: function getProps() {
        return this.getResource("props", "".concat(this.github, "/flatfiles/hierarchy.json"), function (txt) {
          return renderGenPropHierarchyPage(txt);
        });
      }
    }]);

    return GenomePropertiesWebsite;
  }();

  return GenomePropertiesWebsite;

}());
//# sourceMappingURL=gp-website.js.map
